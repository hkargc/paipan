...
var obb={ //农历基础构件
 ...
 getDayName:function(u,r){ //计算农历节日
  ...
  if(u.Lmc2=='正'){ //最后一月
   if(d=='腊月三十' && u.Ldn==30)  r.A += '除夕 ',r.Fjia = 1;
   if(d=='腊月廿九' && u.Ldn==29)  r.A += '除夕 ',r.Fjia = 1;
   if(d=='腊月廿三') r.B += '小年 ';
  }
  if(u.Ljq){
 ...
 mingLiBaZi:function(jd,J,ob){ //命理八字计算。jd为格林尼治UT(J2000起算),J为本地经度,返回在物件ob中
  ...
  v = int2(k/12+6000000);  ob.bz_jn = this.Gan[v%10]+this.Zhi[v%12];ob.bz_jnny = this.gzny[Math.round((((60 + (v + 1) % 60) % 60) / 2), 0)];
  v = k+2+60000000;        ob.bz_jy = this.Gan[v%10]+this.Zhi[v%12];ob.bz_jyny = this.gzny[Math.round((((60 + (v + 1) % 60) % 60) / 2), 0)];
  v = D - 6 + 9000000;     ob.bz_jr = this.Gan[v%10]+this.Zhi[v%12];ob.bz_jrny = this.gzny[Math.round((((60 + (v + 1) % 60) % 60) / 2), 0)];
  v = (D-1)*12+90000000+SC;ob.bz_js = this.Gan[v%10]+this.Zhi[v%12];ob.bz_jsny = this.gzny[Math.round((((60 + (v + 1) % 60) % 60) / 2), 0)];

  v-= SC, ob.bz_JS = ''; //全天纪时表
  for(i=0; i<13; i++){ //一天中包含有13个纪时
    c = this.Gan[(v+i)%10]+this.Zhi[(v+i)%12]; //各时辰的八字
    if(SC==i) ob.bz_js=c, c = '<font color=red>'+c+'</font>', ob.ny = this.gzny[Math.round((((60 + (v + i + 1) % 60) % 60) / 2), 0)]; //红色显示这时辰
    ob.bz_JS += (i?' ':'') + c;
  }
 },
...
function Dtlpq(pjqn) {
	...
	var s = '';
	if (y < 1281 || (y > 1644 && y < 1665) || y > 1669 || (y == 1644 && Cal_m.value - 0 == 12)) return s;
...
function Lunar(){
 ...
 this.yueLiHTML=function(By,Bm,curJD){
  ...
  for(i=0;i<this.dn;i++){ //遍历本月各日(公历)
    ...
    c2 = '<span' + c2 + '>' + ob.d + '<br/>' + ob.Lday2 + '</span>'; //公历的日名称
