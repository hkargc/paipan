...
class Calendar {
	...
	public function gz2gl($ygz, $mgz, $dgz, $hgz, $yeai, $mx) {
		...
		if ($yeai < -1000 || $yeaf > 3000) {
			//說明大誤差區域:適用於西元-1000年至西元3000年,超出此範圍誤差較大
			$this->logs(1);
			return false;
		}
		...
		for ($m = 0; $m <= $mx - 1; $m++) {
			...
			if ($fd < $ijd || $id > $fjd) {
				...
				if ($id < $ijd && $fd > $ijd) {
					//同一个时辰跨越了节:在節氣月頭,只包含時辰後段
					$ids = $ijd;
					$fds = $fd;
				}
				if ($id < $fjd && $fd > $fjd) {
					//同一个时辰跨越了节:在節氣月尾,只包含時辰前段
					$ids = $id;
					$fds = $fjd;
				}
	...
	public function fatemaps($xb, $yy, $mm, $dd, $hh, $mt = 0, $ss = 0) {
		...
		$ewxdz = []; //各地支对应的五行
		for ($k = 0; $k <= 3; $k++) {
			//yytg:八字各柱天干之陰陽屬性,yydz:八字各柱地支之陰陽屬性，nyy[0]為陽之總數，nyy[1]為陰之總數
			$yytg[$k]       = $tg[$k] % 2;
