...
class paipan {
	 ...
	 */
	public $czs = ["长生", "沐浴", "冠带", "临官", "帝旺", "衰", "病", "死", "墓", "绝", "胎", "养"];
	public $yyss = ['異', '同'];
	...
	public function GetGZ($yy, $mm, $dd, $hh, $mt = 0, $ss = 0) {
		...
		$spcjd = (float) $this->Jdays($yy, $mm, $dd, $hh, $mt, $ss);
	 ...
	 * @return false/array
	 */
	public function fatemaps($xb, $yy, $mm, $dd, $hh, $mt = 0, $ss = 0, $J = null, $W = null, $op = []) {

		$op = (array) $op;

		$spcjd = (float) $this->Jdays($yy, $mm, $dd, $hh, $mt, $ss); //special jd,这里依然是标准时间
		...
		for ($k = 0; $k <= 3; $k++) {
			...
			foreach ($this->zcg[$o['dz'][$k]] as $j => $zcg) {
				$o['bzcg'][$k][$j] = $this->ctg[$zcg];
				$o['wxcg'][$k][$j] = $this->wxtg[$zcg];
				$o['yycg'][$k][$j] = $zcg % 2;
				$o['sscg'][$k][$j] = $this->sss[$this->dgs[$zcg][$o['tg'][2]]];
			}
		}
		...
		$qyjd = $spcjd + ($y + $m / 12 + $d / $this->ty) * $this->ty; //转按回归年算
		$qyjt = $this->Jtime($qyjd);

		$o['qyy'] = $qyjt[0]; //起運年(公历)

		$qjr = $this->GetAdjustedJQ($o['qyy'] - 1, false); //立春在上一年的以春分开始的数组中
		...
		$o['qyg_desc'] = "每逢 {$this->ctg[$qyg[0]]}、{$this->ctg[$qyg[1]]} 年{$qyjt[1]}月{$qyjt[2]}日{$qyjt[3]}时交脱大运"; //与十三行八字一致

		$qage = $jqyy - $ob['ty']; //节气年年龄

		$o['dy'] = $o['ly'] = $o['lm'] = $o['lr'] = $o['ls'] = []; //大运,流年,流月,流日,流时
		$o['dyk'] = $o['lyk'] = $o['lmk'] = $o['lrk'] = $o['lsk'] = -1; //默认选中

		$mgz = ((10 + $o['tg'][1] - $o['dz'][1]) % 10) / 2 * 12 + $o['dz'][1]; //这里是根据天干地支代码计算月柱的六十甲子代码
		...
		[$tx, $dx] = $this->GetGZ($year, $month, $day, $hour, 0, 0); //当前四柱
		...
		for ($k = 0; $k <= 8; $k++) { //求各階段的起迄歲數及該階段的大運
			...
			$a['tg'] = ($mgz + 60 + pow(-1, $forward) * ($k + 1)) % 10;
			$a['dz'] = ($mgz + 60 + pow(-1, $forward) * ($k + 1)) % 12;
			...
			$a['zs'] = (24 + $szs[$o['tg'][2]] + pow(-1, $o['tg'][2]) * ($o['dz'][1] + pow(-1, 2 * $forward) * ($a['dz'] - $o['dz'][1]))) % 12;
			$a['czs'] = $this->czs[$a['zs']];

			foreach ($this->zcg[$a['dz']] as $zcg) {
				$a['bzcg'][] = $this->ctg[$zcg];
				$a['wxcg'][] = $this->wxtg[$zcg];
				$a['yycg'][] = $zcg % 2;
				$a['sscg'][] = $this->sss[$this->dgs[$zcg][$o['tg'][2]]];
			}
			$o['dy'][$k] = $a;
			...
			for ($i = 0; $i <= 9; $i++) {
				...
				$b['zs'] = (24 + $szs[$o['tg'][2]] + pow(-1, $o['tg'][2]) * ($o['dz'][1] + pow(-1, 2 * $forward) * ($b['dz'] - $o['dz'][1]))) % 12;
				$b['czs'] = $this->czs[$b['zs']];

				foreach ($this->zcg[$b['dz']] as $zcg) {
					$b['bzcg'][] = $this->ctg[$zcg];
					$b['wxcg'][] = $this->wxtg[$zcg];
					$b['yycg'][] = $zcg % 2;
					$b['sscg'][] = $this->sss[$this->dgs[$zcg][$o['tg'][2]]];
				}
				$o['ly'][$i] = $b;
				...
				$qjr = [];
				for ($ty = $b['year'] - 1; $ty <= $b['year']; $ty++) { //立春开始的
					$dj = $this->GetAdjustedJQ($ty, false);
				...
				for ($j = 0; $j < 12; $j++) {
					...
					list($c['yy'], $c['mm'], $c['dd']) = $this->Jtime($qjr[2 * $j + 21]);

					list($tg, $dz) = $this->GetGZ($c['yy'], $c['mm'], $c['dd'], 24, 0, 0);
					...
					if (isset($op['lmk'])) {
						...
						if (($tx[0] == $tg[0]) && ($tx[1] == $tg[1]) && (abs($c['yy'] - $year) < 10)) { //现在的这个月
							$o['lmk'] = $j;
						}
					}
					...
					foreach ($this->zcg[$c['dz']] as $zcg) {
						$c['bzcg'][] = $this->ctg[$zcg];
						$c['wxcg'][] = $this->wxtg[$zcg];
						$c['yycg'][] = $zcg % 2;
						$c['sscg'][] = $this->sss[$this->dgs[$zcg][$o['tg'][2]]];
					}
					$o['lm'][$j] = $c;
					...
					for ($r = 0, $jd = floor($qjr[2 * $j + 21] + 0.5); $jd < floor($qjr[2 * $j + 21 + 2] + 0.5); $r++, $jd++) { //逐日
						$d = array();

						list($d['yy'], $d['mm'], $d['dd']) = $this->Jtime($jd);
						...
						list($tg, $dz) = $this->GetGZ($d['yy'], $d['mm'], $d['dd'], 0, 0, 0);
						...
						foreach ($this->zcg[$d['dz']] as $zcg) {
							$d['bzcg'][] = $this->ctg[$zcg];
							$d['wxcg'][] = $this->wxtg[$zcg];
							$d['yycg'][] = $zcg % 2;
							$d['sscg'][] = $this->sss[$this->dgs[$zcg][$o['tg'][2]]];
						}
						$o['lr'][$r] = $d;
						...
						for ($x = 0, $z = 0; $x < 24; $x += $step, $z++) {
							$e = array();

							$step = in_array($x, [0, 23], true) ? 1 : 2; //覆盖0点到24点

							list($tg, $dz) = $this->GetGZ($d['yy'], $d['mm'], $d['dd'], $x, 0, 0);
							...
							foreach ($this->zcg[$e['dz']] as $zcg) {
								$e['bzcg'][] = $this->ctg[$zcg];
								$e['wxcg'][] = $this->wxtg[$zcg];
								$e['yycg'][] = $zcg % 2;
								$e['sscg'][] = $this->sss[$this->dgs[$zcg][$o['tg'][2]]];
							}
							$o['ls'][$z] = $e;
						}
					}
		...
		$o['cnl'] = $this->dxs($o['nl'][0]) . "年" . ($o['nl'][3] ? "闰" : "") . "{$this->dxy[$o['nl'][1] - 1]}月{$this->dxd[$o['nl'][2] - 1]}日";
		...
		$lx = floor($this->dzs[$o['dz'][1]][$o['tg'][2]] / 2);
		$o['lx'] = $lx; //类型
		$o['clx'] = $this->lx[$lx]; //类型
		...
		$o['cyy'] = $this->cyy[$yytg[2]]; //日干阴阳
		$o['gx'] = $this->GetGX($o['tg'], $o['dz']); //关系

		return $o;
	}

	public function dxs($number) {
		...
		foreach ($this->gx as $gx) { //[0针对天干1针对地支, 关系类型, [发起者...], 形成者, 文字描述]
			...
			if (empty(array_diff($gx[2], $fd))) { //说明存在此关系
				...
				foreach ($fds as $fd) { //组合成期望的返回
					...
					array_push($list[$gx[0]], [$fd, array_keys($fd), $gx]);
				}
			}
		}
		return $list;
