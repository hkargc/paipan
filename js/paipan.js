...
function paipan() {
	...
	/**
	 * 节气修复 XiuFu 使农历-720年至2300年与寿星万年历匹配,键分别为公历年份和该年自春分开始的廿四节气(鉴于800年前的农历太复杂,暂屏蔽800年前的部分)
	 */
	this.jqXFu = {
		/*'-680':{9:-1},
		'-643':{14:-1},
	...
	/**
	 * 朔望月moon修复 XiuFu 使农历-720年至2300年与寿星万年历匹配,键分别为公历年份和该年第几个朔望日(鉴于800年前的农历太复杂,暂屏蔽800年前的部分)
	 */
	this.smXFu = {
		/*618:{13:-1},
		620:{13:-1},
    ...
    this.qiKB = [ //气直线拟合参数 使农历-720年至2300年与寿星万年历匹配(鉴于800年前的农历太复杂,暂屏蔽800年前的部分)
	...
	this.suoKB = [ //朔直线拟合参数 使农历-720年至2300年与寿星万年历匹配(鉴于800年前的农历太复杂,暂屏蔽800年前的部分)
    ...
    this.Solar2Lunar = function(yy, mm, dd) {
		...
		if (yy >= -721 && yy <= -104) { //这一段来自寿星万年历,十九年七闰法：7个闰年均匀安插在19个年整数中，闰年的末月置为闰月。
			...
			for (var i = 0,step = 3; i <= step; i++) { //计算连续的正月初一,对应的农历日期必定在此范围内
				...
				if (YY >= -220) { //秦汉历,19年7闰,年首为十月,平年九个月,闰年的末月置闰并取名"后九"月,1640641为历法生效时间公历-221.10.31
					var jd = 1640641 + Math.floor(0.866 + (YY + 220) * 12.369000) * this.synmonth; //颁行历年首
					ns[i + 6] = 10; //本项目9为十月this.dxy
				}else if (YY >= -479) { //战国历,19年7闰,年首为正月,平年十二个月,闰年的末月置闰并取名闰"十三",1546083为历法生效时间公历-480.12.11
					var jd = 1546083 + Math.floor(0.500 + (YY + 479) * 12.368422) * this.synmonth;
					ns[i + 6] = 13; //本项目0为正月this.dxy
				}else if (YY >= -722) { //春秋历,19年7闰,年首为正月,平年十二个月,闰年的末月置闰并取名闰"十三",1457698为历法生效时间公历-722.12.17
			...
			if(tjd.length == 13){ //该年有闰月
				ry = (mi == ns[ii + 6]) ? true : false;
				mi = ry ? mi - 1 : mi;
			}
			return Array(yi, mi, dz, ry);
		}
		var flag = 0; //是否是公历年份的前一年
        var sjd = this.GetSMsinceWinterSolstice(yy, true); //求出以含冬至中氣為陰曆11月(冬月)開始的連續16個朔望月的新月點
        var mc = this.GetZQandSMandLunarMonthCode(yy);
    ...
    this.GetXZ = function(yy, mm, dd, hh, mt, ss) {
        ...
        //顯示星座,根据公历的中气判断
        var zr = this.GetZQsinceWinterSolstice(yy, false);
        if (spcjd < zr[0]) {
            var zr = this.GetZQsinceWinterSolstice(yy - 1, false);
        } //若小於雨水，則歸前一年
