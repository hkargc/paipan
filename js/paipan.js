...
function paipan() {
	 ...
	 */
	this.dt_T = function (jd) {
		var y = (jd - 2451545)/365.2425 + 2000; //公历年份,2451545为2000年1月1日12:00:00的jd值
		...
		if (y >= y0) {
			...
			if (y > y0 + 100) {
				return (-20 + jsd * Math.pow((y - 1820) / 100, 2));
			}
			var v = (-20 + jsd * Math.pow((y - 1820) / 100, 2)); //二次曲线外推
			var dv = (-20 + jsd * Math.pow((y0 - 1820) / 100, 2)) - t0; //ye年的二次外推与te的差
	 ...
	 * @param t 儒略世纪数
	 */
	this.XL_calc = function (t) {
		t /= 10; //转为儒略千年数
		var n = 5; //n计算项数
		var i, j, v = 0,
			tn = 1,
			c;
		var F = new Array( //Dear精度:J2000+-4千年 黄经0.1角秒 黄纬0.1角秒 距离0.1AU/10^6
				10000000000, //A的倍率
		...
		var t2 = t * t;
		var t3 = t2 * t; //千年数的各次方
		return v + (-0.0728 - 2.7702 * t - 1.1019 * t2 - 0.0996 * t3) / (180 * 3600 / Math.PI);
	};
	/**
	 * 根据标准时间计算真太阳时,采用低精度算法计算时差,误差约在1秒以内
	 * @param jd 标准时间jd值
	 * @param j 经度(角度表示),东经120度25分为: 120 + 25/60;
	 */
	this.zty = function(jd, j) { //时差计算(低精度),误差约在1秒以内
		var jd = jd - this.TZ / 24; //转为格林尼治UT时间
		var jd2 = jd - 2451545 + this.dt_T(jd) / 86400; //力学时
		var t = jd2 / 36525; //力学时儒略世纪数

		var L = (1753470142 + 628331965331.8 * t + 5296.74 * t * t) / 1000000000 + Math.PI;
		var z = new Array();
		var E = (84381.4088 - 46.836051 * t) / (180 * 3600 / Math.PI);
		z[0] = this.XL_calc(t) + Math.PI;
		z[1] = 0; //地球坐标

		var r = new Array(); //球面坐标旋转,黄道赤道坐标变换,赤到黄E取负       
		r[0] = Math.atan2(Math.sin(z[0]) * Math.cos(E) - Math.tan(z[1]) * Math.sin(E), Math.cos(z[0]));
		r[1] = Math.asin(Math.cos(E) * Math.sin(z[1]) + Math.sin(E) * Math.cos(z[1]) * Math.sin(z[0]));
    ...
    this.GetAdjustedJQ = function(yy, ini, num) {
		...
		for (var i = (ini + 1); i <= (ini + num); i++) {
			...
			jdjq[i] = jdjq[i] + this.TZ / 24; //因中國時間比格林威治時間先行8小時，即1/3日(东经120度)
		}
    ...
    this.GetSMsinceWinterSolstice = function(yy) {
        ...
        for (i = 0; i <= 19; i++) { //求出連續20個朔望月
            k = kn + i;
            tjd[i] = this.TrueNewMoon(k) + this.TZ / 24; //以k值代入求瞬時朔望日,因中國比格林威治先行8小時，加1/3天 (东经120度)
     ...
     * @param int xb 性别0男1女
     * @param int yy 年份.默认传的是this.TZ对应的时间
     ...
     * @return false/array
     */
    this.fatemaps = function(xb, yy, mm, dd, hh, mt, ss, j) {
